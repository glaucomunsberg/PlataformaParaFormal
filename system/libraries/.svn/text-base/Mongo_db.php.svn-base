...
class Mongo_db {
	...
	*	--------------------------------------------------------------------------------
	*
	*	Updates a single document. Manually write the update command
	*
	*	@usage: $this->mongo_db->update('foo', array('$set' => array('foo'=>'bar')));
	*/
	
	public function update_raw($collection = "", $update = array(), $options = array())
	{
		if (empty($collection))
		{
			show_error("No Mongo collection selected to update", 500);
		...
		{
			$options = array_merge($options, array($this->query_safety => TRUE, 'multiple' => FALSE));
			$this->db->{$collection}->update($this->wheres, $update, $options);
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
