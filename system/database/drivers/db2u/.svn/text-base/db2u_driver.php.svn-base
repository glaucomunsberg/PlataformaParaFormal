...
/**
 * DB2 (Uncataloged) Database Adapter Class
 *
 ...
 */
class CI_DB_db2u_driver extends CI_DB {
	...
	function db_pconnect()
	{
		$conn_string = 'DRIVER={IBM DB2 ODBC DRIVER};'
			...
			. 'UID=' . $this->username . ';'
			. 'PWD=' . $this->password . ';';
		return @db2_pconnect($conn_string, '', '');
	}
	...
	function _execute($sql)
	{
		...
		db2_set_option($this->conn_id, $options, 1);
	...
	function trans_commit()
	{
		...
		db2_autocommit($this->conn_id, DB2_AUTOCOMMMIT_ON);
