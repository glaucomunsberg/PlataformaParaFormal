<?php
	class GrupoModel extends Model{
		...
		function getGrupos($start, $limit, $idEmpresa = '', $nomeGrupo = '', $data = '') {
			$this->db->select('grupos.id as id, empresas.nome as empresa, grupos.nome_grupo as nome, date_format(grupos.dt_cadastro, \'%d/%m/%Y\') as dt_cadastro', false);
			$this->db->from('grupos');
		...
		function getGrupo($id) {
			$this->db->select('id, empresa_id as ger_empresas_id, nome_grupo as nome, date_format(dt_cadastro, \'%d/%m/%Y\') as dt_cadastro', false);			
			$this->db->where('id', $id);
			return $this->db->get('grupos')->row();
		}
		
		function getGrupoProgramas($idGrupo, $idPai){
			$this->db->select('grupos_programas.id ||\'chr\'||programas.id as id, nome_programa as nome, date_format(dt_entrada, \'%d/%m/%Y\') as dt_entrada, date_format(dt_saida, \'%d/%m/%Y\') as dt_saida', false);
		...
		function getProgramaGrupo($id){
			$this->db->select('id,'.
							  'grupo_id as ger_grupos_id,'.
							  'programa_id as ger_programas_id,'.
							  'ordem,'.
							  'pai,'.
							  'date_format(dt_entrada, \'%d/%m/%Y\') as dt_entrada,'.
							  'date_format(dt_saida, \'%d/%m/%Y\') as dt_saida', false);
			$this->db->where('id', $id);
